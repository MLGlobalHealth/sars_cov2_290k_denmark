...
COVID_VAC <- dbReadTable(conn = conn,
                          name = "",
                          schema = '')
...
general_address <- dbReadTable(conn = conn2,
                              name = "",
                              schema = '') 
...
sequenced_individual_detailed_metadata$age_at_infection <- 
  as.numeric(difftime(sequenced_individual_detailed_metadata$date, 
                      sequenced_individual_detailed_metadata$BIRTHDAY, units = "days")) / 365.25
...
gg_denmark <- ggplot() +
  ...
  coord_sf(xlim = c(denmark_bbox$xmin, denmark_bbox$xmax), 
...
create_point_map <- function(data, xlim, ylim, region_color_mapping) {
  ...
  gg_map <- ggplot() +
    ...
    xlim(xlim) + ylim(ylim) +  # Set the plot limits
