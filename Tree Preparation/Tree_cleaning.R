...
library('ape', 'adegenet', 'TreeTools', 'tidyverse', 'phangorn', 'patchwork', 
...
merged_metadata <- merge(summary_data, metadata, by = "ConsensusFilename", all.x = TRUE, all.y = TRUE)
...
merged_metadata <- merge(merged_metadata, consensus_pango, by = "strain", all.x = TRUE, all.y = TRUE)
...
chronumental_date_predicted_merged_data$predicted_date <- decimal_date(as.Date(chronumental_date_predicted_merged_data$predicted_date))
chronumental_date_predicted_merged_data$date <- decimal_date(as.Date(chronumental_date_predicted_merged_data$date))
