...
# Importing packages ----------
library(ape, phytools, TreeTools, dplyr, tidyverse, 
...
# All sequenced individuals, regardless of whether we have full metadata for them or not -------------------
...
length(all_sequenced_individuals$PERSON_ID[!is.na(all_sequenced_individuals$PERSON_ID)]) # 293287 individuals in total
length(all_sequenced_individuals_combined_unique$PERSON_ID[!is.na(all_sequenced_individuals$PERSON_ID)]) #292481 unique individuals
...
sequenced_individual_detailed_metadata <- sequenced_individual_detailed_metadata %>%
  mutate(BOP_VFRA = as.Date(BOP_VFRA),
...
all_positive_individuals <- COVID_TEST %>%
          filter(!is.na(PERSON_ID), 
            between(PRDATE, as.Date("2021-01-01"), as.Date("2021-12-31"))) %>% filter(SVARRESULTAT == 1)
...
all_positive_PCR_individuals_missing_ID <- all_positive_individuals %>%
  filter(CASEDEF == "SARS2") %>% filter(SVARRESULTAT == 1)
...
# Keep only the rows with the first/lowest 'PRDATE' within each 'PERSON_ID' group - i.e, only unique individuals
positive_individual_detailed_metadata <- positive_individual_detailed_metadata %>%
  distinct(PERSON_ID, .keep_all = TRUE)
...
all_sequenced_individuals_combined_unique$KOEN <- as.factor(all_sequenced_individuals_combined_unique$KOEN)
...
# Region ----
positive_individual_detailed_metadata$REGIONSKODE <- as.factor(positive_individual_detailed_metadata$REGIONSKODE)
sequenced_individual_detailed_metadata$REGIONSKODE <- as.factor(sequenced_individual_detailed_metadata$REGIONSKODE)
...
# Age Distribution
all_sequenced_individuals_combined$age_group <- cut(all_sequenced_individuals_combined$age_at_infection,
                                                    breaks = c(0, 15, 30, 45, 60, 75, Inf),
                                                    labels = c("0-15", "15-30", "30-45", "45-60", "60-75", "75+"),
                                                    include.lowest = TRUE)

all_sequenced_individuals_combined$age_group <- factor(all_sequenced_individuals_combined$age_group, levels = c("0-15", "15-30", "30-45", "45-60", "60-75", "75+"))
all_sequenced_individuals_combined <- all_sequenced_individuals_combined[!is.na(all_sequenced_individuals_combined$age_group),]
...
sequenced_individual_detailed_metadata <- sequenced_individual_detailed_metadata[!is.na(sequenced_individual_detailed_metadata$region),]
region_plot <- ggplot(sequenced_individual_detailed_metadata, aes(x = region, fill = region)) +
  geom_bar(show.legend = FALSE) +
...
major_variant_plot <- ggplot(sequenced_individuals, aes(x = major_variant, fill = major_variant)) +
  geom_bar(show.legend = FALSE) +
  scale_fill_viridis(discrete = TRUE, option = "magma") +  # Use magma viridis colors for fill
